    ...
    class GameSetting
    {
        ...
        public void ChangeValue(int change, int chosenLine, int tiles, int mines, Action Reprint)
        {
            ...
            else if (Setting.Text.EndsWith("tiles: ")) //Pokud se jedná o nastavení políček
            {
                ...
                { }
                else if ((Setting.Text == "Number of horizontal tiles: ") && (SettingValue.Number + change) > (Console.WindowWidth - (2 * 56))) //Zároveň nesmí být horizontálních políček tolik, že by se hrací plocha s okolními grafikami nevešla na obrazovku
                { }
